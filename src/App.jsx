...
function App() {
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      ...
      // lấy url từ trang https://developer.themoviedb.org/reference/movie-top-rated-list
      const url1 =
        "https://api.themoviedb.org/3/movie/popular?language=vi&page=1";
      const url2 =
        "https://api.themoviedb.org/3/movie/top_rated?language=vn&page=1";
      // Truyền url vào:
      const [res1, res2] = await Promise.all([
        fetch(url1, options),
        fetch(url2, options),
      ]);
      // set data cho cái data1 và data2
